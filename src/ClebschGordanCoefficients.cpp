...
long double FactorialDelta(double j1, double j2, double j3){
  ...
  result = std::sqrt((std::tgamma(1 + j1 + j2 - j3) * std::tgamma(1 + j1 - j2 + j3) * std::tgamma(1 + -j1 + j2 + j3))/(std::tgamma(1 + j1 + j2 + j3 + 1)));
  return result;
}
...
bool Wigner3jSelectionRule(double j1, double j2, double j3, double m1, double m2, double m3){
  ...
  if (!(InVector(m1Values, m1) && InVector(m2Values, m2) && InVector(m3Values, m3))){
    return false;
  }
  else if (m1 + m2 + m3 != 0){
    return false;
...
long double Wigner3j(double j1, double j2, double j3, double m1, double m2, double m3){
  ...
  if (Wigner3jSelectionRule(j1, j2, j3, m1, m2, m3)){
    for (double k = minSum; k <= maxSum; k++){
      sumResult += std::pow(-1, k)/(std::tgamma(k + 1) * std::tgamma(j1 + j2 - j3 - k + 1) * std::tgamma(j1 - m1 - k + 1) * std::tgamma(j2 + m2 - k + 1) * std::tgamma(j3 - j2 + m1 + k + 1) * std::tgamma(j3 - j1 - m2 + k + 1));
    }
    result = Delta(m1 + m2 + m3, 0) * std::pow(-1, j1 - j2 - m3);
    result *= std::sqrt((std::tgamma(j1 + j2 - j3 + 1) * std::tgamma(j1 - j2 + j3 + 1) * std::tgamma(-j1 + j2 + j3 + 1))/std::tgamma(j1 + j2 + j3 + 2));
    result *= std::sqrt(std::tgamma(j1 - m1 + 1) * std::tgamma(j1 + m1 + 1) * std::tgamma(j2 - m2 + 1) * std::tgamma(j2 + m2 + 1) * std::tgamma(j3 - m3 + 1) * std::tgamma(j3 + m3 + 1));
    result *= sumResult;
  }
  else {
    result = 0.0;
...
bool TriangularInequality(double j1, double j2, double j3){
  return (j3 <= j1 + j2 && j3 >= std::abs(j1 - j2) && j2 <= j1 + j3 && j2 >= std::abs(j1 - j3) && j1 <= j3 + j2 && j1 >= std::abs(j3 - j2));
}

bool Wigner6jSelectionRule(double j1, double j2, double j3, double j4, double j5, double j6){
  return (TriangularInequality(j1, j2, j3) && TriangularInequality(j3, j4, j5) && TriangularInequality(j1, j5, j6) && TriangularInequality(j2, j4, j6) && IsInteger(j1 + j2 + j3) && IsInteger(j3 + j4 + j5) && IsInteger(j1 + j5 + j6) && IsInteger(j2 + j4 + j6));
}
...
long double Wigner6j(double j1, double j2, double j3, double j4, double j5, double j6){
  ...
  if (Wigner6jSelectionRule(j1, j2, j3, j4, j5, j6)){
    long double term1 = FactorialDelta(j1, j2, j3) * FactorialDelta(j3, j4, j5) * FactorialDelta(j1, j5, j6) * FactorialDelta(j2, j4, j6);
    long double term2 = 0;
    double startValue = std::max(j1 + j2 + j3, std::max(j3 + j4 + j5, std::max(j1 + j5 + j6, j2 + j4 + j6)));
    double endValue = std::min(j1 + j2 + j4 + j5, std::min(j1 + j3 + j4 + j6, j2 + j3 + j5 + j6));
    for (double n = startValue; n <= endValue; n++){
      long double subTerm = (std::pow(-1, n) * std::tgamma(2 + n)) / (std::tgamma(1 + j1 + j3 + j4 + j6 - n) * std::tgamma(1 + j2 + j3 + j5 + j6 - n) * std::tgamma(1 + n - j1 - j2 - j3) * std::tgamma(1 + n - j3 - j4 - j5) * std::tgamma(1 + n - j1 - j5 - j6) * std::tgamma(1 + n - j2 - j4 - j6) * std::tgamma(1 + j1 + j2 + j4 + j5 - n));
      term2 += subTerm;
    }
    result = term1 * term2;
  }
...
// long double G(double l1, double l2, double l3, double l, double m1, double m2, double m3, double m, double N){
//   long double result = std::pow(-1, l);
//   long double summationTerm = 0;
//   for (double l4 = 0; l4 <= N - 1; l4++){
//     for (double m4 = -l4; m4 <= l4; m4++){
//       summationTerm += std::pow(-1, l4) * std::sqrt((2 * l1 + 1) * (2 * l2 + 1) * (2 * l3 + 1) * (2 * l4 + 1)) * Wigner6j(l1, l2, l4, (N - 1)/2, (N - 1)/2, (N - 1)/2) * Wigner6j(l4, l3, l, (N - 1)/2, (N - 1)/2, (N - 1)/2) * ClesbchGordan(l4, l1, l2, m4, m1, m2) * ClesbchGordan(l, l4, l3, m, m4, m3);
//     }
//   }
//   return result;
// }

// long double K(double l1, double l2, double l3, double l4, double m1, double m2, double m3, double m4, double N){
//   long double result;
//   if (m1 + m2 + m3 + m4 != 0){
//     result = 0;
//   }
//   else {
//     result = std::sqrt((2 * l1 + 1) * (2 * l2 + 1) * (2 * l3 + 1) * (2 * l4 + 1));
//     long double summationTerm = 0;
//     for (double l = 0; l <= N - 1; l++){
//       for (double m = -l; m <= l; m++){
//         summationTerm += ClesbchGordan((N - 1)/2, l1, (N - 1)/2, m + m1, m1, m) * ClesbchGordan((N - 1)/2, l2, (N - 1)/2, m + m1 + m2, m2, m + m1) * ClesbchGordan((N - 1)/2, l3, (N - 1)/2, m + m1 + m2 + m3, m3, m + m1 + m2) * ClesbchGordan((N - 1)/2, l4, (N - 1)/2, m, m4, m + m1 + m2 + m3);
//       }
//     }
//   }
//   return result;
...
long double HFunction(double l1, double l2, double l3, double l, double m1, double m2, double m3, double m, int N){
  ...
  for (double l4 = 0; l4 <= N; l4++){
    for (double m4 = -l4; m4 <= l4; m4++){
      result += std::pow(-1, l1 + l2 + l4) * std::pow(-1, l3 + l4 + l) * std::sqrt((2 * l1 + 1) * (2 * l2 + 1) * (2 * l3 + 1) * (2 * l4 + 1)) * Wigner6j(l1, l2, l4, (N - 1)/2.0, (N - 1)/2.0, (N - 1)/2.0) * Wigner6j(l3, l4, l, (N - 1)/2.0, (N - 1)/2.0, (N - 1)/2.0) * ClesbchGordan(l4, l1, l2, m4, m1, m2) * ClesbchGordan(l, l3, l4, m, m3, m4);
    }
  }
  return result;
}
...
long double G(double a, double b, double c, double d){
  long double result = -Epsilon(a, b)*SigmaX(c, d) + Delta(a, b)*SigmaZ(c, d);
  return result;
}

long double HFunctionInt(int l1_int, int l2_int, int l3_int, int l_int, int m1_int, int m2_int, int m3_int, int m_int, int N){
